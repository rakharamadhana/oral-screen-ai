...
- **Input:** `input_image`, shape `[1, 3, 224, 224]` (resize shorter side to `imgSize`, then centre-crop the square -- no stretch, no pad)
- **Outputs:** `logits` `[1, 1]` and `features` `[1, 2048, 7, 7]`
- **Probability:** `sigmoid(logits[0])` = P(PERLU RUJUKAN)
- **Decision threshold:** `0.2173` (tuned for high sensitivity; read from config)
